...
arg_parser.add_argument(
    ...
    "--val-data-dir",
    type=str,
    default="../point_cloud/val",
    help="Directory that contains validation .h5 files.",
)
arg_parser.add_argument(
    "-c", "--subclass", type=int, default=14, help="Subclass label ID to train on."
)  # 14 is `chair` class.
arg_parser.add_argument(
    "-m",
    "--checkpoint-path",
...
arg_parser.add_argument(
    "-l", "--logdir", type=str, default="logs", help="Directory for saving logs."
)
arg_parser.add_argument(
    "-k",
    "--n-keypoint",
...
arg_parser.add_argument(
    "-d", "--device", type=str, default="cpu", help="Pytorch device for training."
)
...
def feed(net, optimizer, x_set, train, shuffle, batch, epoch, writer):
    running_loss = 0.0
    running_lrc = 0.0
    running_ldiv = 0.0
    net.train(train)
    ...
    with contextlib.suppress() if train else torch.no_grad():
        ...
        # Save to tensorboard
        writer.add_scalar(
            "Loss/train" if train else "Loss/val", running_loss / (i + 1), epoch
        )
        writer.add_scalar(
            "Fidelity loss/train" if train else "Fidelity loss/val",
            running_lrc / (i + 1),
            epoch,
        )
        writer.add_scalar(
            "Coverage loss/train" if train else "Coverage loss/val",
            running_ldiv / (i + 1),
            epoch,
        )
...
if __name__ == "__main__":
    ...
    batch = ns.batch
    x, xl = all_h5(
        DATASET, True, True, subclasses=(ns.subclass,), sample=None
    )  # n x 2048 x 3
    x_test, xl_test = all_h5(
        TESTSET, True, True, subclasses=(ns.subclass,), sample=None
    )
    ...
    for epoch in range(ns.epochs):
        feed(net, optimizer, x, True, True, batch, epoch, writer)
        feed(net, optimizer, x_test, False, False, batch, epoch, writer)
        torch.save(
            {
                "epoch": epoch,
